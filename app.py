...
# 🔄 TWO-WAY SYNC ENGINE (NOW USES URL)
def sync_with_google_sheets(local_dataframe, sheet_url):
...
with st.sidebar:
    ...
    st.info("To sync to the cloud, create a Google Sheet and share it as an 'Editor' with:\n\n**research-bot@dotted-ranger-490614-n9.iam.gserviceaccount.com**")
    ...
    if st.button("🌐 TWO-WAY SYNC (Pull & Push Google Sheets)", type="primary", use_container_width=True):
        with st.spinner("Connecting to Google Cloud..."):
            if not user_sheet_url:
                st.error("❌ Please paste your Google Sheet URL in the sidebar first!")
            else:
                try:
                    merged_data = sync_with_google_sheets(st.session_state.master_database, user_sheet_url)
                    st.session_state.master_database = merged_data
                    st.success("✅ Sync Complete! The cloud and your app are now identical.")
                except Exception as e:
                    st.error(f"❌ Sync Failed. Make sure you shared the sheet with the bot email! Error: {e}")
...
if not st.session_state.master_database.empty:
    ...
    st.info("The database is currently empty. Process a patient or click Sync to download existing cloud data.")import streamlit as st
import pandas as pd
import numpy as np
import base64
import json
