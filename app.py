...
# 🔄 TWO-WAY SYNC ENGINE (NOW USES URL + TAB)
...
with col_x:
    if st.button("🌐 TWO-WAY SYNC (Pull & Push Google Sheets)", type="primary", use_container_width=True):
        with st.spinner("Connecting to Google Cloud..."):
            ...
            elif not project_tab:
                st.error("❌ Please enter a Project Tab Name!")
            else:
                try:
                    ...
                    st.success(f"✅ Sync Complete! Data routed securely to the '{project_tab}' project.")
                except Exception as e:
                    st.error(f"❌ Sync Failed. Check URL and ensure the tab '{project_tab}' exactly matches! Error: {e}")
